...
public class ForwardingService implements Task, ManagedLifecycle {
     ...
     * queue regardless of their origin
     * If true, it will only process messages that were processed by a
     * MessageStore running on the same server
            ...
            try {
                ...
                while (!isSuccessful && !isTerminated) {
                    try {
                        ...
                        isSuccessful = true; // isSuccessfull is true even
                                             // session is not available because
                                             // of avoiding the unwanted retries
                    } catch (Exception e) {
