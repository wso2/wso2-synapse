...
public class EIPUtils {
	 ...
	 * 
	 * @param envelope
	 *            SOAPEnvelope from which the elements will be extracted
	 * @param expression
	 *            SynapseXPath expression describing the elements to be
	 *            extracted
	...
	/**
	 * Return the set of detached elements specified by the XPath over the given
	 * envelope
	 * 
	 * @param envelope
	 *            SOAPEnvelope from which the elements will be extracted
	...
	/**
	 * Merge two SOAP envelopes using the given XPath expression that specifies
	 * the
	 * element that enriches the first envelope from the second
	 * 
	 * @param envelope
	 *            SOAPEnvelope to be enriched with the content
	 * @param enricher
	 *            SOAPEnvelope from which the enriching element will be
	 *            extracted
	 * @param expression
	 *            SynapseXPath describing the enriching element
	 * @throws JaxenException
	...
	public static void enrichEnvelope(SOAPEnvelope envelope, SOAPEnvelope enricher, MessageContext synCtxt,
		...
		if ((checkNotEmpty(elementList) && checkNotEmpty(list)) || (!checkNotEmpty(elementList) && checkNotEmpty(list))) {
			if (checkNotEmpty(elementList)) {
				// attach at parent of the first result from the XPath, or to
				// the SOAPBody
	...
	private static boolean isBody(OMElement body, OMElement enrichingElement) {
		try {
			return (body.getLocalName().equals(enrichingElement.getLocalName()) && body.getNamespace()
			                                                                           .getNamespaceURI()
			                                                                           .equals(enrichingElement.getNamespace()
			                                                                                                   .getNamespaceURI()));
	...
	public static void createSynapseEIPTemplateProperty(MessageContext synCtxt, String templateName, String paramName,
	                                                    Object value) {
	 ...
	 * 
	 * @param envelope
	 *            SOAPEnvelope which is to be enclosed
	 * @param encloseElement
	 *            enclosing element
	...
	public static void includeEnvelope(SOAPEnvelope envelope, SOAPEnvelope enricher, MessageContext synCtxt,
		...
		//SynapseXPath mergeExpression = SynapseXPathFactory.getSynapseXPath(elem, expression)
				
				
	    SynapseXPath parentExpression = new SynapseXPath("//ns:getOrderResponse/ns:return");
