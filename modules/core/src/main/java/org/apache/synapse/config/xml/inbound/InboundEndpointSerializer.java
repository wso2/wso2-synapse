    ...
    public static OMElement serializeInboundEndpoint(InboundEndpoint inboundEndpoint) {
        ...
        for (Map.Entry<String, String> paramEntry : inboundEndpoint.getParametersMap().entrySet()) {
            OMElement parameter = fac.createOMElement(InboundEndpointConstants.INBOUND_ENDPOINT_PARAMETER,
                    SynapseConstants.SYNAPSE_OMNAMESPACE);
            parameter.addAttribute(InboundEndpointConstants.INBOUND_ENDPOINT_PARAMETER_NAME, paramEntry.getKey(), null);
        ...
        OMElement sequenceElt = fac.createOMElement(InboundEndpointConstants.INBOUND_ENDPOINT_SEQ,
                SynapseConstants.SYNAPSE_OMNAMESPACE);
        sequenceElt.addAttribute(InboundEndpointConstants.INBOUND_ENDPOINT_SEQ_KEY, inboundEndpoint.getInjectingSeq(), null);
        sequenceElt.addAttribute(InboundEndpointConstants.INBOUND_ENDPOINT_ON_ERROR_SEQ, inboundEndpoint.getOnErrorSeq(), null);

        inboundEndpointElt.addChild(sequenceElt);
