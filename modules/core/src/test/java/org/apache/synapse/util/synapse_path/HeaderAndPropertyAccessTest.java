...
public class HeaderAndPropertyAccessTest {
    ...
    public void testTransportHeader() throws Exception {
        Synapse_Path testPath = new Synapse_Path("headers.test");
    ...
    public void testTransportHeaderWithReservedName() throws Exception {
        Synapse_Path testPath = new Synapse_Path("toLower(headers.[\"toUpper\"])");
        Assert.assertEquals("hello world", testPath.stringValueOf(synCtx));
    }

    @Test
    public void testTransportHeaderInFilter() throws Exception {
        Synapse_Path testPath = new Synapse_Path("$..book[?(@.price > integer(headers.price))].title");
    ...
    public void testAxis2HeaderInFilter() throws Exception {
        Synapse_Path testPath = new Synapse_Path("$..book[?(@.category == attributes.axis2.category)].title");
    ...
    public void testAxis2HeaderWithReservedName() throws Exception {
        Synapse_Path testPath = new Synapse_Path("attributes.axis2.[\"empty\"]");
    ...
    public void testSynapseHeader() throws Exception {
        Synapse_Path testPath = new Synapse_Path("attributes.synapse.phrase");
    ...
    public void testSynapseHeaderWithFunction() throws Exception {
        Synapse_Path testPath = new Synapse_Path("length(split(attributes.synapse.phrase, \" \"))");
        Assert.assertEquals("9", testPath.stringValueOf(synCtx));
    }

    @Test
    public void testSynapseHeaderInFilter() throws Exception {
        Synapse_Path testPath = new Synapse_Path("$..book[?(@.title == attributes.synapse.[\"selected book\"])].price");
        Assert.assertEquals("[7.99]", testPath.stringValueOf(synCtx));
    }

    @Test
    public void testNonExistingEmptyAndNull() throws Exception {
        Synapse_Path testPath = new Synapse_Path("attributes.synapse.nonExisting");
        Assert.assertEquals(null, testPath.stringValueOf(synCtx));
        testPath = new Synapse_Path("attributes.synapse.[\"null\"]");
        Assert.assertEquals(null, testPath.stringValueOf(synCtx));
        testPath = new Synapse_Path("attributes.synapse[\"empty\"]");
